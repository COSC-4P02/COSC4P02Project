...
Connection.prototype.checkHandshake = function (lines) {
  ...
  if (!lines[0].match(/^HTTP\/\d\.\d 101( .*)?$/i)) {
    this.emit(
      "error",
      new Error("Invalid handshake: invalid first line format")
    );
    return false;
  }
  ...
  if (
    ...
    this.emit(
      "error",
      new Error("Invalid handshake: missing required headers")
    );
    return false;
  }
  if (
    ...
    this.emit(
      "error",
      new Error("Invalid handshake: invalid Upgrade or Connection header")
    );
    return false;
  }
  ...
  if (this.protocols && this.protocols.length) {
    ...
    if (!protocol || this.protocols.indexOf(protocol) === -1) {
      this.emit(
        "error",
        new Error("Invalid handshake: no protocol was negotiated")
      );
      return false;
    }
  } else {
    ...
    if (protocol) {
      this.emit(
        "error",
        new Error("Invalid handshake: no protocol negotiation was expected")
      );
      return false;
    }
  }
