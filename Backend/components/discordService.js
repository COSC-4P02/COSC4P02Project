  ...
  client.on("messageCreate", async function (message) {
    try {
      ...
      if (command === "brock" || message.channelId === config.discordBrock) {
        if (command === "brock") message.content = message.content.substring(7);
        check = "brock";
        stats_one(print, errorlog, dbMain, "discord/msg");
      } else if (
        command === "game" ||
        message.channelId === config.discordGame
      ) {
        if (command === "game") message.content = message.content.substring(6);
        check = "game";
        stats_one(print, errorlog, dbMain, "discord/msg");
      }
