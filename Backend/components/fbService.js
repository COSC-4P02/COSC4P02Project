...
async function nlp_msg(message,print, errorlog, chatlog, threshold, nlpManager, dbMain, dbCache, senderPsid, version){
...
async function handleMessage(version, senderPsid, receivedMessage, print, errorlog, chatlog, nlp_info, dbMain, dbCache) {
      let response;
      ...
      if (receivedMessage.text) {
        ...
        var msg = "";
        if (version === "brock")
            msg = await nlp_msg(receivedMessage.text,print, errorlog, chatlog, threshold, nlpManagerBrock, dbMain, dbCache,senderPsid, version);
        else if  (version === "game")
            msg = await nlp_msg(receivedMessage.text,print, errorlog, chatlog, threshold, nlpManagerGame, dbMain, dbCache,senderPsid, version);
        response = {
          'text': msg
        };
        ...
        response = {
          'attachment': {
            ...
            'payload': {
              ...
              'elements': [{
                'title': 'Sorry, but I cant read attachments, do you want me to send it to my developer?',
    ...
    function callSendAPI(senderPsid, response, version) {
      ...
      if (version === "brock")
        PAGE_ACCESS_TOKEN = config.FB_brock.pageToken;
      else if  (version === "game")
        PAGE_ACCESS_TOKEN = config.FB_game.pageToken;
...
module.exports = function (version, print, errorlog, chatlog, nlp_info, dbMain, dbCache, req, res) {
    ...
    if (body.object === 'page') {
        ...
        body.entry.forEach(function(entry) {
          ...
          if (webhookEvent.message) {
            handleMessage(version, senderPsid, webhookEvent.message, print, errorlog, chatlog, nlp_info, dbMain, dbCache);
