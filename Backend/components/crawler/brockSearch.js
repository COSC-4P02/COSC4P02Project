...
function requestSearch(
  ...
  axios
    ...
    .then(function (res) {
      ...
      if ("error" in data) {
        errorlog("Crawler: Brock Search Error - " + parma);
        result([]);
        return;
      }

      if (!("results" in data)) {
        print("Crawler: Brock Search Not Found - " + parma);
        result([]);
        const db_search_loc =
          "/crawler/data/brock/search/array/" +
      ...
      all_searchs.push(first);
      print("Crawler: Brock Search - " + parma);
      result(all_searchs);
      const db_search_loc =
        "/crawler/data/brock/search/array/" +
        encodeURIComponent(parma.replace(" ", "+").replace("/", ""));
    ...
    .catch(function (error) {
      console.log(error);
      if (cse_token != "AJvRUv1zOSA2wllMyve3JOmVxBrE:1650220582426")
        requestSearch(
          cx,
          "AJvRUv1zOSA2wllMyve3JOmVxBrE:1650220582426",
          api,
          parma,
          errorlog,
          result,
...
module.exports = function (parma, dbMain, print, errorlog, dbCache, result) {
  stats_one(print, errorlog, dbMain, "api/crawler/brockSearch/search");
  const db_search_loc =
    "/crawler/data/brock/search/array/" +
    encodeURIComponent(parma.replace(" ", "+").replace("/", ""));
  ...
  try {
    ...
    if (rss_cache.length > 0) {
      print("Crawler: Brock Search Read from database");
      result(rss_cache);
      return;
    }
  } catch (e) {
  ...
  const cx = "017436347380509209892:ehplam6quxc";
